   ...
   useEffect(() => {
    ...
    const handleApi = async () => {
        ...
        if (json) {
            ...
            const prioridade = ["GERENTE_PREVENCAO", "ADMIN"];

            const perfilPrioritario =  prioridade.includes(usuario?.perfil)

            const acessos = json.acess;

            if (!perfilPrioritario && acessos.length === 1) {
                navigate(acessos[0].router, { replace: false });
                return;
            }

            if (perfilPrioritario) {
                navigate("/required/geral", { replace: false });
                return;
            }
